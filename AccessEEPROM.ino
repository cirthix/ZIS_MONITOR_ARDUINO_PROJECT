...
void UserConfiguration_SaveBacklightMode(uint8_t myValue){ EEPROM.update(ADDRESS_BACKLIGHT_MODE, myValue); }
void UserConfiguration_SaveDefaultBacklightMode(){EEPROM.update(ADDRESS_BACKLIGHT_MODE, FACTORY_DEFAULT_BACKLIGHT_MODE); }

uint8_t UserConfiguration_LoadBrightnessStable(){return EEPROM.read(ADDRESS_BACKLIGHT_LEVEL_STABLE);}
void UserConfiguration_SaveBrightnessStable(uint8_t myValue){ EEPROM.update(ADDRESS_BACKLIGHT_LEVEL_STABLE, myValue); }
void UserConfiguration_SaveDefaultBrightnessStable(){EEPROM.update(ADDRESS_BACKLIGHT_LEVEL_STABLE, FACTORY_DEFAULT_BACKLIGHT_STABLE_BRIGHTNESS); }

uint8_t UserConfiguration_LoadBrightnessPulse(){return EEPROM.read(ADDRESS_BACKLIGHT_LEVEL_PULSE);}
void UserConfiguration_SaveBrightnessPulse(uint8_t myValue){ EEPROM.update(ADDRESS_BACKLIGHT_LEVEL_PULSE, myValue); }
void UserConfiguration_SaveDefaultBrightnessPulse(){EEPROM.update(ADDRESS_BACKLIGHT_LEVEL_PULSE, FACTORY_DEFAULT_BACKLIGHT_PULSE_BRIGHTNESS); }

uint8_t UserConfiguration_LoadFrequencyPWM(){return EEPROM.read(ADDRESS_PWM_FREQUENCY);}
void UserConfiguration_SaveFrequencyPWM(uint8_t myValue){ EEPROM.update(ADDRESS_PWM_FREQUENCY, myValue); }
void UserConfiguration_SaveDefaultFrequencyPWM(){EEPROM.update(ADDRESS_PWM_FREQUENCY, FACTORY_DEFAULT_BACKLIGHT_PULSE_BRIGHTNESS); }

uint8_t UserConfiguration_LoadWasSlave(){return EEPROM.read(ADDRESS_WAS_SLAVE);}
void UserConfiguration_SaveWasSlave(uint8_t myValue){ EEPROM.update(ADDRESS_WAS_SLAVE, myValue); }
void UserConfiguration_SaveDefaultWasSlave(){EEPROM.update(ADDRESS_WAS_SLAVE, false); }
...
void UserConfiguration_SaveTestMode(uint8_t myValue){ EEPROM.update(ADDRESS_USE_OCTESTMODE, myValue); }
void UserConfiguration_SaveDefaultTestMode(){EEPROM.update(ADDRESS_USE_OCTESTMODE, FACTORY_DEFAULT_USE_OCTESTMODE); }
